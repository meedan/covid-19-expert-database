...
module Jekyll
  require 'date'
    ...
    class DataPage < Page
      ...
      # - `data_index` is the index of the record
      # - `name` is the key in `data` which determines the output filename
      # - `name_expr` is an expression for generating the output filename
      # - `template` is the name of the template for generating the page
      # - `extension` is the extension for the generated file
      def initialize(site, base, index_files, dir, data, data_index, name, name_expr, template, extension)
        @site = site
        @base = base
          ...
          data.each do |data_spec|
            ...
            if site.layouts.key? template
              ...
              records.each_with_index do |record, index|
                site.pages << DataPage.new(site, site.source, index_files_for_this_data, dir, record, index, name, name_expr, template, extension)
              end
            else
